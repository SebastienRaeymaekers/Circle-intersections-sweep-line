...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    public Node getParentNodeOf(Key key) {
        return getParentNodeOf(root, key);
    }

    private Node getParentNodeOf(Node x, Key key) {
        if (key == null) throw new IllegalArgumentException("calls get() with a null key");
        if (x == null) return null;
        int cmp = key.compareTo(x.key); // if we seek parent node of root, return null
        if(cmp == 0) return null;
        int cmpLeft = key.compareTo(x.left.key); // compare key to key of left child
        if      (cmpLeft < 0) return getNode(x.left, key);
        else if (cmpLeft > 0) return getNode(x.right, key);
        else if (cmpLeft == 0) return x.left;
        int cmpRight = key.compareTo(x.right.key); // compare key to key of right child
        if      (cmpRight < 0) return getNode(x.left, key);
        else if (cmpRight > 0) return getNode(x.right, key);
        else return x.right;
    }
